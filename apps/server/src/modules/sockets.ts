import { ChangeLanePayload, CLIENT_EVENTS, EjectionPayload, InputPayload, LeaderboardPayload, Server as GameInstance, SERVER_EVENTS, StatesPayload, TRACKS } from "@vroom/shared";
...
class Sockets {
  ...
  private startGameInstance(roomId: string, trackName: string): void {
    ...
    const instance = (this.rooms[roomId].game = new GameInstance(
      ...
      (id: string, eventName: SERVER_EVENTS, payload: StatesPayload | LeaderboardPayload | EjectionPayload) => this.emit(id, eventName, payload),
      TRACKS[trackName]
    ));
